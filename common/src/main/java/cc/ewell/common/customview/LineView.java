...
import cc.ewell.common.utils.LineViewUtils;
...
public class LineView extends View {
    ...
    private final int bottomTriangleHeight = 6;
    public boolean showPopup = true;

    private Dot selectedDot;

    private int topLineLength = LineViewUtils.dip2px(getContext(), 12);
    ; // | | ←this
    //-+-+-
    private int sideLineLength = LineViewUtils.dip2px(getContext(), 45) /  2;// --+--+--+--+--+--+--
    //  ↑this
//    private int backgroundGridWidth = LineViewUtils.dip2px(getContext(),25);
    private int backgroundGridWidth;

    //Constants
    private final int popupTopPadding = LineViewUtils.dip2px(getContext(), 1);
    private final int popupBottomMargin = LineViewUtils.dip2px(getContext(), 1);
    private final int bottomTextTopMargin = LineViewUtils.sp2px(getContext(), 5);
    private final int bottomLineLength = LineViewUtils.sp2px(getContext(), 22);
    private final int DOT_INNER_CIR_RADIUS = LineViewUtils.dip2px(getContext(), 2);
    private final int DOT_OUTER_CIR_RADIUS = LineViewUtils.dip2px(getContext(), 4);
    private final int MIN_TOP_LINE_LENGTH = LineViewUtils.dip2px(getContext(), 12);
...
//    private String[] colorArray = {"#e74c3c","#2980b9","#1abc9c"};
    private String[] colorArray = {"#468fff", "#468fff", "#468fff"};
    //popup 컬러
//    private int[] popupColorArray = {R.drawable.popup_red,R.drawable.popup_blue,R.drawable.popup_green};
//    private int[] popupColorArray = {R.color.color_ffffffff,R.color.color_ffffffff,R.color.color_ffffffff};
//    private int[] popupColorArray = {Color.parseColor("#ffffff"),Color.parseColor("#ffffff"),Color.parseColor("#ffffff")};

    private int[] popupColorArray;


    public interface LineViewCallBack {
        public void onClickDot(int position);
    }

    LineViewCallBack lineViewCallBack = null;

    public void setOnClickDot(LineViewCallBack iCallBack) {
        lineViewCallBack = iCallBack;
    }
    ...
    public void setPopupColorArray(int[] popupColorArray) {
        this.popupColorArray = popupColorArray;
    }
...
//        popupTextPaint.setColor(Color.WHITE);
        popupTextPaint.setColor(Color.parseColor("#468fff"));
        popupTextPaint.setTextSize(LineViewUtils.sp2px(getContext(), 12));
        popupTextPaint.setStrokeWidth(5);
        popupTextPaint.setTextAlign(Paint.Align.CENTER);

        bottomTextPaint.setAntiAlias(true);
        bottomTextPaint.setTextSize(LineViewUtils.sp2px(getContext(), 12));
        bottomTextPaint.setTextAlign(Paint.Align.CENTER);
        bottomTextPaint.setStyle(Paint.Style.FILL);
        bottomTextPaint.setColor(BOTTOM_TEXT_COLOR);
    }

    public void setBackgroundGridWidth(int width) {
        backgroundGridWidth = (width - sideLineLength * 2) / 11;
    }
        ...
        if (dataLists != null && !dataLists.isEmpty()) {
            ...
            for (int k = 0; k < dataLists.size(); k++) {
                ...
                for (int i = 0; i < dataLists.get(k).size(); i++) {
                    ...
                    if (i > drawDotSize - 1) {
                        //도트리스트를 추가한다.
                        drawDotLists.get(k).add(new Dot(x, 0, x, y, dataLists.get(k).get(i), i, (checkPosition == i) ? true : false));
                    } else {
                        //도트리스트에 타겟을 설정한다.
                        drawDotLists.get(k).set(i, drawDotLists.get(k).get(i).setTargetData(x, y, dataLists.get(k).get(i), i, (checkPosition == i) ? true : false));
                    }
                }
...
// 선택한 dot 만 popup 이 뜨게 한다.        
    ...
    private void drawPopup(Canvas canvas, String num, Point point, int PopupColor) {
        ...
        int sidePadding = LineViewUtils.dip2px(getContext(), singularNum ? 8 : 5);
        int x = point.x;
        int y = point.y - LineViewUtils.dip2px(getContext(), 5);
...
//        popup.setBounds(r);
        ...
        Drawable view = getResources().getDrawable(PopupColor);
        view.draw(canvas);
...
//        popup.draw(canvas);
    ...
    private void drawDots(Canvas canvas) {
        ...
        circlePaint.setColor(Color.parseColor("#D9E9FF"));
        ...
        if (drawDotLists != null && !drawDotLists.isEmpty()) {
            for (int k = 0; k < drawDotLists.size(); k++) {
                ...
                for (Dot dot : drawDotLists.get(k)) {
                    ...
                    if (dot.isCheck) {
                        canvas.drawCircle(dot.x, dot.y, DOT_OUTER_CIR_RADIUS + 4, circlePaint);
                        bigCirPaint.setColor(Color.parseColor("#468fff"));
                        bigCirPaint.setStrokeWidth(DOT_OUTER_CIR_RADIUS);
...
//                    canvas.drawCircle(dot.x, dot.y, DOT_INNER_CIR_RADIUS, smallCirPaint);
    ...
    private void drawLines(Canvas canvas) {
        ...
        linePaint.setStrokeWidth(LineViewUtils.dip2px(getContext(), 1));
        for (int k = 0; k < drawDotLists.size(); k++) {
    ...
    private void drawBackgroundLines(Canvas canvas) {
        ...
        paint.setStrokeWidth(LineViewUtils.dip2px(getContext(), 1f));
...
//      }
                ...
                for (Dot dot : data) {
                    ...
                    if (r.contains(point.x, point.y) && event.getAction() == MotionEvent.ACTION_DOWN) {
                        ...
                        if (r.contains(point.x, point.y)) {
                            showPopup = true;
                            checkPosition = selectedDot.getLinenumber();
                            lineViewCallBack.onClickDot(selectedDot.linenumber);
    ...
    class Dot {
        ...
        int velocity = LineViewUtils.dip2px(getContext(), 18);

        Dot(int x, int y, int targetX, int targetY, Integer data, int linenumber, boolean isCheck) {
            this.x = x;
            this.y = y;
            this.linenumber = linenumber;
            this.isCheck = isCheck;
            setTargetData(targetX, targetY, data, linenumber, isCheck);
        }
        ...
        Dot setTargetData(int targetX, int targetY, Integer data, int linenumber, boolean isCheck) {
